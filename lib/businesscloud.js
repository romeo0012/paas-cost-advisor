...
// Business Cloud (T-Business) IaaS is billed as a "Resource Pool":
// total = Σ CPU GHz × rate + Σ RAM GB × rate + Σ disk GB × tier rate.
...
// Remote backup and public IP are NOT billed in this model.
...
// Remote backup and public IP are not billed (kept as 0 for API compatibility).
function summarize(nodes, cm = 12, remoteBackupGB = null) {
  ...
  totals.totalCZK = totals.cpuCostCZK + totals.ramCostCZK + totals.diskCostCZK
  return {
    commitmentMonths: cm,
