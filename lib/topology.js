...
function priceTopology(topologyInput, currency = 'CZK', utilizationPct, commitmentMonths, rates) {
  ...
  const conv = (a, from, to) => convert(a, from, to, rates)
  ...
  for (const pp of perProvider) {
    ...
    pp.totalCZK = round2(conv(pp.totalUSD, 'USD', 'CZK'))
    pp.total = round2(conv(pp.totalUSD, 'USD', currency))
    pp.compute = round2(conv(pp.computeUSD, 'USD', currency))
    pp.storage = round2(conv(pp.storageUSD, 'USD', currency))
  }
  ...
  const perNode = nodes.map((n, i) => {
    ...
    return {
      ...
      businessCloud: {
        ...
        total: round2(conv(bcNode.totalCZK, 'CZK', currency)),
      },
      hyperscalers: Object.fromEntries(perProvider.map(pp => {
        ...
        return [pp.id, m ? {
          ...
          totalUSD: m.totalUSD,
          total: round2(conv(m.totalUSD, 'USD', currency)),
        } : null]
      })),
      paas: Object.fromEntries(Object.keys(paasCatalog).map(pid => {
        ...
        return [pid, {
          ...
          totalCZK,
          total: round2(conv(totalCZK, 'CZK', currency)),
        }]
      })),
    }
  })
  ...
  const totals = {
    businessCloud: {
      totalCZK: bcTotal,
      total: round2(conv(bcTotal, 'CZK', currency)),
    ...
    perProvider: perProvider.map(pp => ({
      ...
      compute: pp.compute,
      storage: pp.storage,
      savingPct: bcTotal > 0 ? Math.round((1 - conv(pp.totalUSD, 'USD', 'CZK') / bcTotal) * 100) : 0,
    })),
    paas: Object.fromEntries(Object.keys(paasCatalog).map(pid => {
      const totalCZK = round2(perNode.reduce((s, n) => s + (n.paas[pid] ? n.paas[pid].totalCZK : 0), 0))
      return [pid, { totalCZK, total: round2(conv(totalCZK, 'CZK', currency)) }]
    })),
  }
