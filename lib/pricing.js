...
async function refreshProviderPrices(providerId) {
  ...
  for (let i = 0; i < prov.plans.length; i++) {
    ...
    if (providerId === 'aws') {
      ...
      if (m) {
        if (!awsData) {
          const url = `https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonEC2/current/${prov.region}/index.json`
          const res = await fetchWithTimeout(url, 60000)
          awsData = await res.json()
        }
...
module.exports = { refreshProviderPrices }
