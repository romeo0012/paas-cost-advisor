(function () {
  ...
  const translations = {
    cs: {
      ...
      'rateCpu': 'CPU (Kč/GHz/měs)',
      'rateRam': 'RAM (Kč/GB/měs)',
      'noCommitment': 'bez závazku',
      'diskSuperFast': 'Super Fast',
      'diskFast': 'Fast',
      'diskStandard': 'Standard',
      'diskBasic': 'Basic',
      'bcPoolExtra': 'Remote backup 0,68 Kč/GB/měs na 2× celkový disk; public IP 108 Kč/měs při přítomnosti opnsense firewallu.',
      'hsMatchingTitle': 'AWS / GCP / Azure – IaaS matching',
      'hsMatchingText': 'Pro každé VM se vybere nejlevnější instance splňující vCPU i RAM: vCPU = max(1, ceil(cpuGHz / 2,6)), RAM = max(1, ceil(ramGB)); cena = USD/hod × 730 + diskGB × storage sazba.',
      ...
      'availableInstancesText': 'On-demand ceny (USD/hod). Zdroj: data/instances.json.',
      'storageRates': 'Storage sazby (USD/GB/měs)',
    ...
    en: {
      ...
      'rateCpu': 'CPU (CZK/GHz/mo)',
      'rateRam': 'RAM (CZK/GB/mo)',
      'noCommitment': 'no commitment',
      'diskSuperFast': 'Super Fast',
      'diskFast': 'Fast',
      'diskStandard': 'Standard',
      'diskBasic': 'Basic',
      'bcPoolExtra': 'Remote backup 0.68 CZK/GB/mo applied to 2× total disk; public IP 108 CZK/mo when an opnsense firewall is present.',
      'hsMatchingTitle': 'AWS / GCP / Azure – IaaS matching',
      'hsMatchingText': 'For each VM the cheapest instance that fits both vCPU and RAM is chosen: vCPU = max(1, ceil(cpuGHz / 2.6)), RAM = max(1, ceil(ramGB)); price = USD/hr × 730 + diskGB × storage rate.',
      ...
      'availableInstancesText': 'On-demand prices (USD/hr). Source: data/instances.json.',
      'storageRates': 'Storage rates (USD/GB/mo)',
      'colInstance': 'Instance',
      'colVcpu': 'vCPU',
      'colRam': 'RAM (GiB)',
      'colUsdHour': 'USD/hr',
